...
Note: see Section 6.7 of the SAPA book"
  ...
  (let* ((N-acvs (1+ max-lag))
    ...
    (cond
     ...
     (t 
      (setf (aref scratch-dft 0) (aref acvs 0)) ;;correction made by karim 
      ;added code to set lag window at zero to unity
      (let ((tau 1)
  ...
  "given
   ...
   [3] sample-size (required)
       ==> length of the time series from which precolored-sdf-estimate
           was cons (lag-window-spectral-estimate
                        the-acvs 
                        #'(lambda (tau)
                            (parzen-lag-window
                             tau 5))
                        :N-nonzero-freqs :Fourier
                        :sampling-time sampling-time)tructed; this is needed to get corresponding
           grid of frequencies
